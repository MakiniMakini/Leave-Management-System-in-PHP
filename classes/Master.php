...
Class Master extends DBConnection {
	...
	// create a function to save the contract type
	function save_contract(){
		...
		$check = $this->conn->query("SELECT * FROM `contract_list` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
		...
		if(empty($id)){
			$sql = "INSERT INTO `contract_list` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `contract_list` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
	...
	// create a function to delete contract
	function delete_contract(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `contract_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function save_application(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Leave date has conflict to other leave applications. Please review and try again.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
...
switch ($action) {
	...
	break;
	case 'save_contract':
		echo $Master->save_contract();
	break;
	case 'delete_contract':
		echo $Master->delete_contract();
	break;
