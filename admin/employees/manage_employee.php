...
$contract_qry = $conn->query("SELECT id,name FROM contract_list");
$contract_arr = array_column($contract_qry->fetch_all(MYSQLI_ASSOC),'name','id');
?>
			...
			<form action="" id="manage-user">	
				...
				<div class="row">
					...
					<div class="col-6">
						...
						<!-- added the contract type -->
						<div class="form-group">
							<label for="contract_id">Contract Type</label>
							<select name="contract_id" id="contract_id" class="form-control select2bs4 select2 rounded-0" data-placeholder="Please Select Contract type here" reqiured>
								<option value="" disabled <?php echo !isset($meta['contract_id']) ? 'selected' : '' ?>></option>
								<?php foreach($contract_arr as $k=>$v): ?>
									<option value="<?php echo $k ?>" <?php echo (isset($meta['contract_id']) && $meta['contract_id'] == $k) ? 'selected' : '' ?>><?php echo $v ?></option>
								<?php endforeach; ?>
							</select>
						</div>
						<!-- added the date of employment -->
						<div class="form-group">
							<label for="doe">Date of Employment</label>
							<input type="date" name="doe" id="doe" class="form-control rounded-0" value="<?php echo isset($meta['doe']) ? date("Y-m-d",strtotime($meta['doe'])): '' ?>" required>
						</div>
						<div class="form-group">
