...
<script>
	...
	if ($('#date_start').val() != '') {
		...
		// Exclude weekends from the date calculation if working days are selected
		...
		if ($('#type').val() == 2) {
		// If working days are selected, divide the count by 2 since half days are allowed
		days = days / 2;
		}

		$('#leave_days').val(days.toFixed(1));
	}
	}
